...
class AgendarCitaPantalla extends GetView<AgendarCitaControlador> {
              ...
              children: [
                ...
                Expanded(
                  child: controller.slots.isEmpty
                      ? const Center(child: Text('Selecciona médico y fecha'))
                      : Wrap(
                          spacing: 8,
                          runSpacing: 8,
                          children: controller.slots.map((s) => _slotChip(s)).toList(),
                        ),
                ),
  ...
  Widget _slotChip(Slot s) {
    ...
    return Obx(() => ChoiceChip(
      label: Text(label),
      selected: selected,
      onSelected: s.disponible ? (v) {
        ctrl.slotSel.value = v ? s : null;
      } : null,
